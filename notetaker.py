...
def update_user_bio(conversation_summary, latest_message, core_memories, existing_user_bio):
    try:
        ...
        system_prompt = load_system_prompt('./long-term-memory-system/system_prompt_user_persona_notes.txt')
        system_prompt = {
            "role": "system",
...
def update_conversation_summary(conversation_summary, filtered_messages):
    try:
        ...
        system_prompt = load_system_prompt("./long-term-memory-system/system_prompt_summary_notes.txt")
        system_prompt = {
            "role": "system",
            "content": system_prompt
        }
...
def generate_context_list(conversation_summary, user_input, core_memories):
    try:
        system_prompt = load_system_prompt('./long-term-memory-system/system_prompt_relevant_list_notes.txt')
        system_prompt = {
            "role": "system",
            "content": system_prompt
        }
...
def toggle_personality(core_memories, context_list, user_context_list, user_input, current_personality):
    try:
        system_prompt = load_system_prompt('./long-term-memory-system/system_prompt_personality_switch_notes.txt')
        system_prompt = {
            "role": "system",
            "content": system_prompt
        }
...
def update_core_memory(recent_messages, conversation_summary, existing_core_memories):
    try:
        system_prompt = load_system_prompt('./long-term-memory-system/system_prompt_core_memory_notes.txt')
        system_prompt = {
            "role": "system",
            "content": system_prompt
        }
    ...
    while True:
        ...
        if user_input.lower() != 'read-file':
            ...
            if personality_flag == '<P1>':
                ...
                system_prompt = load_system_prompt('system_prompt_bad_notetaker.txt')
                messages.append({
                    "role": "system",
                    "content": system_prompt
                })
